include conf

ARMGNU = aarch64-none-elf
COPS = -std=c23 -DMAINFSFUNC=$(MAINFSFUNC) -fPIE -Wall -O0 -ffreestanding -nostdlib -nostartfiles -mstrict-align -Ikernel -fno-omit-frame-pointer -mno-omit-leaf-frame-pointer
ASMOPS = -Ikernel -fPIE

BUILD_DIR = build
SRC_DIR = kernel

all : clean kernel8.img

screen:
	scripts/view_tty.sh $(TTYDEV) $(VIEW_TTY)

dump:
	$(ARMGNU)-objdump -D $(BUILD_DIR)/kernel8.elf > dump

clean:
	@rm -rf $(BUILD_DIR) *.img dump

$(BUILD_DIR)/%_c.o: $(SRC_DIR)/%.c
	@mkdir -p $(@D)
	@$(ARMGNU)-gcc $(COPS) -MMD -c $< -o $@

$(BUILD_DIR)/%_s.o: $(SRC_DIR)/%.S
	@mkdir -p $(@D)
	@$(ARMGNU)-gcc $(ASMOPS) -MMD -c $< -o $@

C_FILES = $(shell find . \( -path ./library -o -path ./modules \) -prune -o -type f -name "*.c" -print | cut -d'/' -f2-)
ASM_FILES = $(shell find . \( -path ./library -o -path ./modules \) -prune -o -type f -name "*.S" -print | cut -d'/' -f2-)

OBJ_FILES = $(C_FILES:$(SRC_DIR)/%.c=$(BUILD_DIR)/%_c.o)
OBJ_FILES += $(ASM_FILES:$(SRC_DIR)/%.S=$(BUILD_DIR)/%_s.o)

DEP_FILES = $(OBJ_FILES:%.o=%.d)
-include $(DEP_FILES)

jlink: COPS += -g
jlink: ASMOPS += -DDEBUG_WAIT -g
jlink: clean linker.ld $(OBJ_FILES)
	@$(ARMGNU)-ld -pie -T linker.ld -o $(BUILD_DIR)/kernel8.elf  $(OBJ_FILES)
	@$(ARMGNU)-objcopy $(BUILD_DIR)/kernel8.elf -O binary kernel8.img
	scripts/debug.sh $(RPI_PATH) $(TTYDEV) $(VIEW_TTY) "jlink"

debug: COPS += -g
debug: ASMOPS += -DDEBUG_WAIT -g
debug: clean linker.ld $(OBJ_FILES)
	@$(ARMGNU)-ld -pie -T linker.ld -o $(BUILD_DIR)/kernel8.elf  $(OBJ_FILES)
	@$(ARMGNU)-objcopy $(BUILD_DIR)/kernel8.elf -O binary kernel8.img
	scripts/debug.sh $(RPI_PATH) $(TTYDEV) $(VIEW_TTY) "debug"

kernel8.img: linker.ld $(OBJ_FILES)
	@$(ARMGNU)-ld -pie -T linker.ld -o $(BUILD_DIR)/kernel8.elf  $(OBJ_FILES)
	@$(ARMGNU)-objcopy $(BUILD_DIR)/kernel8.elf -O binary kernel8.img
	scripts/go_rpi.sh $(RPI_PATH) $(TTYDEV) $(VIEW_TTY) "view"
